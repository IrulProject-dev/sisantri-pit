...
<div class="bg-white rounded-xl shadow-sm p-6 border border-blue-100 max-w-3xl mx-auto">
    ...
    <!-- Form -->
    <form action="{{ route('assessment-targets.update', $assessmentTarget) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="santri_id" required
                        ...
                        <option value="{{ $santri->id }}"
                            {{ old('santri_id', $assessmentTarget->santri_id) == $santri->id ? 'selected' : '' }}>
                            {{ $santri->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="assessment_component_id" required
                        ...
                        <option value="{{ $component->id }}"
                            {{ old('assessment_component_id', $assessmentTarget->assessment_component_id) == $component->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="target_score"
                           step="0.01"
                           min="0"
                           value="{{ old('target_score', $assessmentTarget->target_score) }}"
                           class="mt-1 block w-full rounded-md border-blue-200 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 transition duration-200"
                           required>
                    @error('target_score')
                        <p class="mt-1 text-red-500 text-xs italic">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="date" name="target_date"
                           value="{{ old('target_date', $assessmentTarget->target_date->format('Y-m-d')) }}"
                           min="{{ $assessmentTarget->target_date->format('Y-m-d') }}"
                           class="mt-1 block w-full rounded-md border-blue-200 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 transition duration-200"
                           required>
                    @error('target_date')
                        <p class="mt-1 text-red-500 text-xs italic">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="mt-1 block w-full rounded-md border-blue-200 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 transition duration-200"
                          placeholder="Tambahkan catatan (opsional)">{{ old('notes', $assessmentTarget->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="border-t border-blue-100 pt-6">
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i>Perbarui Target
                </button>
            </div>
        </div>
    </form>
